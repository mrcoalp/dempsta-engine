...
class Marshalling {
public:
    static inline int GetValue(MarshallingType<int>, lua_State* L, int index) {
        ensure_type(lua_isinteger(L, index));
        return static_cast<int>(lua_tointeger(L, index));
    }

    static inline float GetValue(MarshallingType<float>, lua_State* L, int index) {
        ensure_type(lua_isnumber(L, index));
        return static_cast<float>(lua_tonumber(L, index));
    }

    static inline double GetValue(MarshallingType<double>, lua_State* L, int index) {
        ensure_type(lua_isnumber(L, index));
        return static_cast<double>(lua_tonumber(L, index));
    }

    static inline bool GetValue(MarshallingType<bool>, lua_State* L, int index) {
        ensure_type(lua_isboolean(L, index));
        return lua_toboolean(L, index) != 0;
    }

    static inline unsigned GetValue(MarshallingType<unsigned>, lua_State* L, int index) {
        ensure_type(lua_isinteger(L, index));
        return static_cast<unsigned>(lua_tointeger(L, index));
    }

    static inline std::string GetValue(MarshallingType<std::string>, lua_State* L, int index) {
    ...
    static inline const char* GetValue(MarshallingType<const char*>, lua_State* L, int index) {
        ensure_type(lua_isstring(L, index));
        return lua_tostring(L, index);
    }

    static inline void* GetValue(MarshallingType<void*>, lua_State* L, int index) {
    ...
    template <typename T>
    static std::vector<T> GetValue(MarshallingType<std::vector<T>>, lua_State* L, int index) {
        ensure_type(lua_istable(L, index));
        ...
        for (size_t i = 1; i <= size; ++i) {
            ...
            vec.emplace_back(GetValue(MarshallingType<T>{}, L, -1));
            lua_pop(L, 1);
        }
        return vec;
    }
    ...
    template <typename T>
    [[nodiscard]] static LuaMap<T> GetValue(MarshallingType<LuaMap<T>>, lua_State* L, int index) {
        ensure_type(lua_istable(L, index));
        lua_pushnil(L);
        LuaMap<T> map;
        while (lua_next(L, -2) != 0) {
            map.emplace(GetValue(MarshallingType<std::string>{}, L, -2), GetValue(MarshallingType<T>{}, L, -1));
            lua_pop(L, 1);
        }
        return map;
    }
